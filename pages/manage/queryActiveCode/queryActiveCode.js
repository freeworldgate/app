...
Page({
  ...
  onLoad: function (options) {
      ...
      var activeCode = wx.getStorageSync('activeCode');
      that.setData({activeCode:activeCode})
  ...
  inputActiveCode:function(res){
    ...
    wx.setStorageSync('activeCode', value)
  },
  ...
  copy:function(){
    ...
    that.setData({code:'等待刷新...'})

  },
  activePK:function(res)
  {
    var that = this;
    // template.createOperateDialog(that).show("获取激活码", "确定获取激活码...", function () {



      var httpClient = template.createHttpClient(that);
      httpClient.setMode("label", true);
      httpClient.send(request.url.queryActiveCode, "GET",{applyCode:that.data.activeCode});   
  ...
  activePK1:function(res)
  {
    var that = this;
    template.createOperateDialog(that).show("登录", "登录获取激活码...", function () {
