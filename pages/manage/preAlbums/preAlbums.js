...
Page({
  ...
  queryPks:function (tab) {
    ...
    httpClient.send(request.url.queryPrePks, "GET", {});
  },
  onShow:function(){
    var that = this;
  ...
  onReachBottom: function () {
    var that = this;
      ...
      httpClient.send(request.url.morePrePks, "GET",{ userId:user.userId ,page:that.data.page});
  ...
  createPk:function()
  {
    ...
    template.createEditPkDialog(that).show(function (topic,watchWord,invite) {
      ...
      httpClient.send(request.url.preCreatePk, "GET",{topic:topic,watchWord:watchWord,invite:invite});
    });
